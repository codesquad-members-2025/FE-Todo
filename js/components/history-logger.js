...
export default function createLogEntry({ actionType, cardId, columnId = '', afterColumnId = '' }) {
    const textInfo = KanbanStore.getTextInfo({ cardId, columnId, afterColumnId });
    const logEntry = {
        iconName: 'flog.png',
        userName: 'Nago',
        text: createText(actionType, textInfo),
        datetime: getISODate(new Date()),
    }
    PanelStore.addLogEntry(logEntry);
}

function createText(actionType, { cardTitle, columnTitle, afterColumnTitle }) {
    return {
